...
import Timeline, { TimelineGroupBase, SidebarHeader, TimelineHeaders, DateHeader } from 'react-calendar-timeline'
...
export default function StaffAttendanceTimeline({ staffList, attendanceList }: StaffAttendanceTimelineProps) {
    ...
    // 그룹 정의 (직원 리스트)
    const groups: TimelineGroupBase[] = staffList.map(s => ({
        id: s.id,
        title: s.name,
        stackItems: true,
    }))
    ...
    // 아이템 정의 (근태 기록)
    const items: TimelineItem[] = (Array.isArray(attendanceList) ? attendanceList : []).map(a => {
