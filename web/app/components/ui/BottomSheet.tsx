...
export function BottomSheet({
  ...
  const { isVisible: isKeyboardVisible } = useVirtualKeyboard({
    onShow: () => {
      // 키보드가 열릴 때 바텀 시트 높이 조정
      if (sheetRef.current) {
        sheetRef.current.style.maxHeight = '50vh' // 키보드 공간 확보
      }
    },
    onHide: () => {
      // 키보드가 닫힐 때 원래 높이로 복원
      if (sheetRef.current) {
        sheetRef.current.style.maxHeight = `${maxHeight}vh`
      }
    },
    enabled: open && mobileOnly,
  })
  ...
  const content = (
    <div
      ...
      style={{
        ...dragStyle,
        maxHeight: isKeyboardVisible ? '50vh' : `${maxHeight}vh`,
        transition: isDragging ? 'none' : 'max-height 0.3s ease-out, transform 0.3s ease-out',
      }}
