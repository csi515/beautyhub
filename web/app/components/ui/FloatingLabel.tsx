    ...
    <div className="relative block">
      <div className="relative">
        <input
          ...
          aria-invalid={!!error}
          aria-describedby={error ? `${(rest as any).id || 'input'}-error` : helpText ? `${(rest as any).id || 'input'}-help` : undefined}
        />
        <label
          htmlFor={(rest as any).id}
          className={clsx(
      ...
      {error ? (
        <div
          id={`${(rest as any).id || 'input'}-error`}
          role="alert"
          className="mt-1 text-xs text-rose-600 help-text-error"
        >
          {error}
        </div>
      ) : helpText ? (
        <div
          id={`${(rest as any).id || 'input'}-help`}
          className="mt-1 text-xs text-neutral-500 help-text"
        >
          {helpText}
        </div>
      ) : null}
