...
export default function DashboardProductList({ products, limit, isTablet }: DashboardProductListProps) {
  ...
  return (
    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: { xs: 2, md: isTablet ? 1.5 : 2 } }}>
      ...
      {sliced.length > 0 ? (
        ...
        <Stack alignItems="center" justifyContent="center" spacing={2} sx={{ py: 5, bgcolor: 'background.default', borderRadius: 3, border: '1px dashed', borderColor: 'divider' }}>
          <PackageOpen size={48} className="text-gray-300" style={{ width: 'clamp(40px, 12vw, 48px)', height: 'clamp(40px, 12vw, 48px)' }} />
          <Box textAlign="center">
            ...
            <Link href="/products" aria-label="상품 추가하기" style={{ color: '#3B82F6', fontSize: '0.875rem', fontWeight: 600, textDecoration: 'none', minHeight: '44px', minWidth: '44px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0.75rem 1rem' }}>
              + 상품 추가하기
            </Link>
          </Box>
        </Stack>
