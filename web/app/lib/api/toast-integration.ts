...
export function useApiErrorToast() {
  ...
  useEffect(() => {
    const handleApiError = (event: CustomEvent<{ message: string; status?: number }>) => {
      ...
      if (status === 401) {
        toast.error(getLocalizedErrorMessage(message, '인증이 필요합니다. 다시 로그인해주세요.'))
        return
      }

      // 404 에러
      if (status === 404) {
        toast.error(getLocalizedErrorMessage(message, '찾을 수 없습니다.'))
        return
      }

      // 400 에러 (검증 오류 등)
      if (status === 400) {
        toast.error(getLocalizedErrorMessage(message, '잘못된 요청입니다.'))
        return
      }

      // 기타 에러
      toast.error(getLocalizedErrorMessage(message, '오류가 발생했습니다.'))
    }
